...
public function get_user_permission_list($permit){
   ...
   if($permit=="Admin")$sql="where permit <=5";
...
public function user_permission_list(){
  ...
  $per[1]="IMNU Manager";
  $per[2]="Admission Manager";
  $per[3]="Fee Manager";
  
  $per[4]="$sort_name Accountant";
  $per[5]="$sort_name Teacher";
  $per[6]="Student";
  $per[7]="$sort_name Workers";
  $per[9]="$sort_name Workers ground";
...
public function get_user_can_update_option($user_role,$login_user_role){
  ...
  foreach ($list as $key => $value) {
    ...
    if($role==$user_role && $role==1)$select="selected";
